...
public abstract class SwipeHelper extends ItemTouchHelper.SimpleCallback {
    ...
    private float swipeThreadhold = 0.5f;
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if(buttonBuffer.containsKey(swipePosition))
            ...
            buttonList.clear();
        buttonBuffer.clear();
        swipeThreadhold = 0.5f*buttonList.size()*buttonWidth;
        recoverSwipeItem();
    }

    public float getSwipeThreadhold(RecyclerView.ViewHolder viewHolder) {
        return swipeThreadhold;
    }
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE)
        {
            if(dX < 0 )
            {
                ...
                trasisionX = dX*buffer.size()*buttonWidth / itemView.getWidth();
                drawButton(c,itemView,buffer,pos,trasisionX);
            }
        }
        super.onChildDraw(c,recyclerView,viewHolder,trasisionX,dY,actionState,isCurrentlyActive);
    }

    public  void drawButton(Canvas c, View itemView, List<MyButton> buffer, int pos, float trasisionX){
        float right = itemView.getRight();
        float dbuttonWidth = -1*trasisionX/buffer.size();
